...
async def extract_updates_from_page(page, case_id, page_num=1):
    ...
    # If no structured updates found, try a broader approach by scanning all content for dollar amounts
    if not page_updates:
        logging.info("No structured updates found, trying general page scan")
        all_text = updates_soup.get_text()
        
        # Find all dollar amounts in the page text
        ...
        for match in dollar_matches:
            ...
            # Only add if it looks like a valid fee (contains some fee keywords)
            fee_keywords = ['fee', 'charge', 'cost', 'invoice', 'bill', 'payment', 'paid', 'due', 'update', 'history']
            if any(keyword in surrounding_text.lower() for keyword in fee_keywords):
                count += 1
                update_entry = {
                    "date": extract_date_from_text(surrounding_text),
                    "details": surrounding_text.strip(),
                    "amount": amount,
                    "amountStr": amount_str,
                    "type": categorize_update(surrounding_text),
                    "source": "page_scan"
                }
                page_updates.append(update_entry)
                
        if count > 0:
            logging.info(f"Found {count} updates using general page scan approach")
        ...
        try:
            ...
            try:
                pagination_info = await page.evaluate("""() => {
                    ...
                    const nextButtons = document.querySelectorAll('a.next, button.next, a[rel="next"]');
                ...
                logging.warning(f"Error during pagination analysis: {str(e)}")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_updates_extraction())
